...
class ChatRequestHandler(BaseHTTPRequestHandler):
    ...
    def handle_login(self, message):
        logger.logger.info(f'Login attempt: {message}')
        # Add the client to the list of connected clients
        print("Handling login:", message)
    ...
    def handle_message(self, message):
        logger.logger.info(f'Message received: {message}')
...
if __name__ == '__main__':
    HOST, PORT = '', 0 # host is empty string to bind to all available interfaces
    server = ChatServer((HOST, PORT), ChatRequestHandler)
    ip, port = server.server_address # Get the IP address and port number the server is bound to
    print(f'Starting server on {ip}:{port}, use <Ctrl-C> to stop')
